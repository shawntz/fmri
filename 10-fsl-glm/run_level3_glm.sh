...
# @Description: Run FSL FEAT Level 3 (group-level) GLM analysis
...
log_file="${SLURM_LOG_DIR}/fsl-glm/${MODEL_NAME}_level3_processing.log"

echo "($(date)) [INFO] Starting FSL FEAT Level 3 analysis for model: ${MODEL_NAME}" | tee -a "${log_file}"
...
MODEL_DIR="${BASE_DIR}/model/level3/model-${MODEL_NAME}"
if [ ! -d "$MODEL_DIR" ]; then
    echo "Error: Model directory does not exist: $MODEL_DIR" | tee -a "${log_file}"
    echo "Please ensure level 2 analysis is complete" | tee -a "${log_file}"
    exit 1
fi
...
echo "($(date)) [INFO] Running level 3 FEAT analysis" | tee -a "${log_file}"

python3 "$(dirname "$0")/run_level3.py" \
...
if [ $? -eq 0 ]; then
    echo "($(date)) [INFO] Successfully completed Level 3 FEAT analysis for model: ${MODEL_NAME}" | tee -a "${log_file}"
else
    echo "($(date)) [ERROR] Level 3 FEAT analysis failed for model: ${MODEL_NAME}" | tee -a "${log_file}"
    exit 1
fi
