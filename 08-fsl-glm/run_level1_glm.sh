...
# @Description: Run FSL FEAT Level 1 (individual runs) GLM analysis
...
log_file="${SLURM_LOG_DIR}/fsl-glm/${MODEL_NAME}_level1_processing.log"

echo "($(date)) [INFO] Starting FSL FEAT Level 1 analysis for model: ${MODEL_NAME}" | tee -a "${log_file}"
...
MODEL_DIR="${BASE_DIR}/model/level1/model-${MODEL_NAME}"
if [ ! -d "$MODEL_DIR" ]; then
    echo "Error: Model directory does not exist: $MODEL_DIR" | tee -a "${log_file}"
    echo "Please run setup_glm.sh first to create the model structure" | tee -a "${log_file}"
    exit 1
fi
...
echo "($(date)) [INFO] Running level 1 FEAT analysis" | tee -a "${log_file}"

python3 "$(dirname "$0")/run_level1.py" \
...
if [ $? -eq 0 ]; then
    echo "($(date)) [INFO] Successfully completed Level 1 FEAT analysis for model: ${MODEL_NAME}" | tee -a "${log_file}"
else
    echo "($(date)) [ERROR] Level 1 FEAT analysis failed for model: ${MODEL_NAME}" | tee -a "${log_file}"
    exit 1
fi
