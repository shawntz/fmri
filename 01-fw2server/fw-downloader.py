...
@Usage: Called by download.sh as part of the preprocessing pipeline
...
class FlywheelDownloader:
    ...
    def _get_api_key(self) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Flywheel CLI API key file not found at {fw_api_key_path}"
            )
            raise
    ...
    def login_to_fw(self) -> bool:
        try:
            ...
            fw_path_os_cmd = (
                f"$HOME/flywheel/cli/fw login {self.fw_instance_url}:{fw_api_key}"
            )
            ...
            self.logger.error(
                f"Login attempt to {self.fw_instance_url} failed: {str(e)}"
            )
            return False
    ...
    def download(self, data_type: str, output_dir: str) -> bool:
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.error(
                    f"{data_type} download failed with return code {result.returncode}"
                )
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download zipped images from flywheel."
    )
    ...
    parser.add_argument(
        "--fw_project_id", action="store", help="Flywheel Project ID (e.g., amass)"
    )
    parser.add_argument(
        "--fw_instance_url",
        action="store",
...
def main():
    ...
    if args.fw_subject_id is None:
        raise ValueError(
            "Flywheel subject ID argument not defined, please do so with the --fw_subject_id flag!"
        )

    if args.fw_session_id is None:
        raise ValueError(
            "Flywheel session ID argument not defined, please do so with the --fw_session_id flag!"
        )

    if args.fw_project_id is None:
        raise ValueError(
            "Flywheel project ID argument not defined, please do so with the --fw_project_id flag!"
        )

    if args.fw_instance_url is None:
        raise ValueError(
            "Flywheel instance server URL argument not defined, please do so with the --fw_instance_url flag!"
        )

    if args.fw_group_id is None:
        raise ValueError(
            "Flywheel group ID argument not defined, please do so with the --fw_group_id flag!"
        )

    if args.fw_api_key_file is None:
        raise ValueError(
            "Flywheel API key file path argument not defined, please do so with the --fw_api_key_file flag!"
        )
